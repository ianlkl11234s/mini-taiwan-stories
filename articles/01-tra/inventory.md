...
### 1. ⚠️ 時刻表轉換器毀了台中線一整段 —— 已修，但只修到未來

> ✅ **2026-08-23 已修復**（data-collectors PR #57 merged、Zeabur 部署並實測驗證）。
> 從 **2026-08-23 起**，`data.schedules[]` 多一個 `stations_raw` 欄位＝TDX 原樣的停靠序列。
> **站級統計一律讀 `stations_raw`，不要讀 `stations`。**
>
> | 站 | `stations` | `stations_raw` | 外部實測 |
> |---|---|---|---|
> | 潭子 | 0 | **114** | **114** ✅ |
> | 嘉北 | 2 | **79** | **79** ✅ |
> | 栗林 | 114（污染） | **102** | **102** ✅ |
>
> ⚠️ **2026-08-22 以前的 167 天沒有這個欄位**，仍然只有壞掉的 `stations`。
> 本文的統計窗口（2026-08-16~22）落在修復前，所以**排除 11 站的做法仍然必要**。
> 要拿回完整歷史需從 S3 歸檔（`rail_timetable/archives/`，2026-02-28 起連續）回填，未做。

以下是根因記錄（`stations` 的行為刻意保留不動，所以這些坑對舊資料永遠成立）：
...
**採用的方式**：`convert_tra_train(..., include_raw_stops=True)` 額外輸出一份未經
站碼映射／軌道過濾／去重的原始停靠序列，**完全不動 `stations`**。
S3 給 3D 前端的路徑維持預設 False，輸出逐位元不變（908 班逐筆比對 0 筆不同）。

選這個而不是直接修 `stations`，是因為 `stations` 有兩個需求相反的消費者：
3D 前端要「軌道上畫得出來的站」，統計要「TDX 說停哪就是哪」。additive 的改法不需要
稽核下游有誰在讀。實測 DB 增量 241kB → 384kB／日（約 +52 MB/年）。

**仍未做（留給後續）**：
1. 移除 `STATION_ID_MAPPING` 裡已過期的 `'3340':'3330'`（會改到 S3 輸出，另開 PR）
2. `find_matching_track` 配不到而整班丟掉的 2 班（910→908），動它會改 `train_count`
3. 歷史 167 天從 S3 回填
4. 新馬（7140）多抓幾天歸檔確認 TDX 是否長期缺
